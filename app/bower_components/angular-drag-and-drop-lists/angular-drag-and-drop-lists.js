...
angular.module('dndLists', [])
    ...
    .directive('dndDraggable', ['$parse', '$timeout', 'dndDropEffectWorkaround', 'dndDragTypeWorkaround',
        ...
        return function(scope, element, attr) {
            ...
            element.on('dragstart', function(event) {
                ...
                event.dataTransfer.setData("Text", JSON.stringify(scope.$eval(attr.dndDraggable)));
                console.log( angular.fromJson(event.dataTransfer.getData("Text")))
     ...
     */
    .directive('dndList', ['$timeout', 'dndDropEffectWorkaround', 'dndDragTypeWorkaround', 'Ticket', 
                   function($timeout,   dndDropEffectWorkaround,   dndDragTypeWorkaround , Ticket ) {
        return function(scope, element, attr) {
            ...
            element.on('drop', function(event) {
                ...
                // Clean up
                placeholder.remove();
                //console.log(listNode.parentNode.previousElementSibling.id)
                
                //targetArray[0]._id.$oid = 
                //console.log(listNode.children)

                targetArray[targetArray.length-1].status = listNode.parentNode.previousElementSibling.id
                var myData = targetArray[targetArray.length-1]
                //console.log(myData, 'mydata')
                Ticket.update({_id: myData._id.$oid}, myData)
